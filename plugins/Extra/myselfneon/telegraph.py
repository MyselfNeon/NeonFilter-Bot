    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=120)) as session:
            with open(file_path, "rb") as f:
                ...
                async with session.post(STREAMABLE_API, data=data, auth=auth) as resp:
                    if resp.status == 200:
...
async def telegraph_start(bot: Client, message: Message):
    ...
    # Only add Streamable button if credentials exist
    if STREAMABLE_USER and STREAMABLE_PASS:
        buttons.append([InlineKeyboardButton("Sᴛʀᴇᴀᴍᴀʙʟᴇ 📺", callback_data="telegraph_streamable")])
...
async def telegraph_callback(bot: Client, query: CallbackQuery):
    ...
    # If user clicks Streamable but credentials not set
    if site == "streamable" and (not STREAMABLE_USER or not STREAMABLE_PASS):
        return await query.answer(
            "❌ Streamable upload is not configured. Please set username and API token in info.py",
            show_alert=True
        )
...
async def telegraph_file_handler(bot: Client, message: Message):
    ...
    # -----------------------------
    # File size check for Catbox & Streamable
    # -----------------------------
    ...
    try:
        ...
        await status_msg.edit_text(
            ...
            disable_web_page_preview=False if site == "streamable" else True,
